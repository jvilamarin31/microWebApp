...
def get_service_config(service_name_default='orders', service_port_default=5004, service_host_default='microorders'):
...
def register_service(app, service_name_default='orders', service_port_default=5004, service_host_default='microorders'):
...
def discover_service(service_name, fallback_url=None):
    """
    Descubre dinamicamente la URL de una instancia saludable del servicio
    preguntando a Consul.
    """
    consul_url = get_consul_url()
    try:
        ...
        if resp.status_code == 200:
            ...
            if instances:
                entry = instances[0]
                addr = entry['Service'].get('Address') or entry['Node'].get('Address')
                port = entry['Service'].get('Port')
                discovered_url = f"http://{addr}:{port}"
                print(f"[Consul Discovery] Servicio '{service_name}' descubierto dinámicamente en {discovered_url}")
                return discovered_url
            else:
                print(f"[Consul Discovery] Advertencia: No hay instancias saludables para '{service_name}' en Consul.")
                return None
        else:
            print(f"[Consul Discovery] Respuesta inesperada de Consul ({resp.status_code})")
    except Exception as e:
        print(f"[Consul Discovery] Error al conectar con Consul en {consul_url}: {e}")
        if fallback_url:
            print(f"[Consul Discovery] Usando fallback local: {fallback_url}")
            return fallback_url
        return None
